            ...
            ->assertInertia(function (Assert $page) use ($flights) {
                $page->component('Flight/Index')
                    ->has('flights', 20, function (Assert $page) {
                        $page->hasAll(['id', 'name', 'origin', 'destination', 'departure', 'arrival', 'seats']);
                    })
                    ...
                    // ->where('flights.0.seats_available', 90) // This is a computed property, so it's not in the database
            });
    }
}